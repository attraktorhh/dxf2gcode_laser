...
class Erstelle_Fenster:
    ...
    def erstelle_menu(self): 
        ...
        self.menu.add_cascade(label=_("File"), underline=0, menu=self.filemenu)
        self.filemenu.add_command(label=_("Open DXF"), underline=0, command=self.Get_Load_File)
        
        self.filemenu.add_separator()
        self.filemenu.add_command(label=_("Exit"), underline=1, command=self.ende)

        self.exportmenu = Menu(self.menu,tearoff=0)
        self.menu.add_cascade(label=_("Export"), underline=0, menu=self.exportmenu)
        self.exportmenu.add_command(label=_("Write G-Code"), underline=0, command=self.Write_GCode)
        #Disabled bis was gelesen wurde
        self.exportmenu.entryconfig(0,state=DISABLED)

        self.viewmenu=Menu(self.menu,tearoff=0)
        self.menu.add_cascade(label=_("View"),underline=0, menu=self.viewmenu)
        ...
        self.viewmenu.add_command(label=_('Autoscale'),underline=1,command=self.Canvas.autoscale)

        #Menupunkt einfuegen zum loeschen der Route
        self.viewmenu.add_separator()
        self.viewmenu.add_command(label=_('Delete Route'),underline=0,command=self.del_route_and_menuentry)         
        ...
        self.menu.add_cascade(label=_("Options"),underline=0,menu=self.optionmenu)
        self.optionmenu.add_command(label=_("Set tolerances"), underline=4, command=self.Get_Cont_Tol)
        self.optionmenu.add_separator()
        self.optionmenu.add_command(label=_("Scale contours"), underline=0, command=self.Get_Cont_Scale)
        self.optionmenu.add_command(label=_("Move workpiece zero"), underline=0, command=self.Move_WP_zero)
        self.optionmenu.add_command(label=_("Rotate contours"), underline=0, command=self.Rotate_Cont)
        ...
        self.menu.add_cascade(label=_("Help"), underline=0, menu=self.helpmenu)
        self.helpmenu.add_command(label=_("About..."), underline=0, command=self.Show_About)
    ...
    def Load_File(self):
        ...
        master.title("%s" %(self.load_filename))
    ...
    def Write_GCode(self):
       ...
       #Config & postpro in einen kurzen Namen speichern
        ...
        for nr in range(1,len(self.TSP.opt_route)):
            ...
            #print "Shape ", nr, "starting ", start.x, start.y

            #Drucken falls die Shape nicht disabled ist
            if not(shape.nr in self.CanvasContent.Disabled) and (shape.nr in self.CanvasContent.CutFirst):
                self.textbox.prt((_("\nWriting Shape (first): %d (%d) at %5.3f %5.3f") %(nr,self.TSP.opt_route[nr],start.x, start.y)))
                #Falls sich die Fräserkorrektur verändert hat diese in File schreiben
                stat =shape.Write_GCode(config,postpro)
                status=status*stat

        self.textbox.prt((_("\nShapes not marked as cut first or last....")))
        for nr in range(1,len(self.TSP.opt_route)):
            shape=self.shapes_to_write[self.TSP.opt_route[nr]]
            start,start_ang=shape.get_st_en_points(0)
            #print "Shape ", nr, "starting ", start.x, start.y
            ...
            #Drucken falls die Shape nicht disabled ist
            if not(shape.nr in self.CanvasContent.Disabled) and not(shape.nr in self.CanvasContent.CutLast) and not(shape.nr in self.CanvasContent.CutFirst):
                self.textbox.prt((_("\nWriting Shape (normal): %d (%d) at %5.3f %5.3f") %(nr,self.TSP.opt_route[nr],start.x, start.y)))
                #Falls sich die Fräserkorrektur verändert hat diese in File schreiben
                stat =shape.Write_GCode(config,postpro)
                status=status*stat

        self.textbox.prt((_("\nShapes marked as cut last....")))
        for nr in range(1,len(self.TSP.opt_route)):
            shape=self.shapes_to_write[self.TSP.opt_route[nr]]
            start,start_ang=shape.get_st_en_points(0)

            if not(shape.nr in self.CanvasContent.Disabled) and (shape.nr in self.CanvasContent.CutLast):
                self.textbox.prt((_("\nWriting Shape (last): %d (%d) at %5.3f %5.3f") %(nr,self.TSP.opt_route[nr],start.x, start.y)))
                #Falls sich die Fräserkorrektur verändert hat diese in File schreiben
                stat =shape.Write_GCode(config,postpro)
                status=status*stat
...
class TextboxClass:
    def __init__(self,frame=None,master=None,DEBUG=1):
...
class CanvasClass:
    ...
    def make_contextmenu(self,event):
        ...
        popup.add_command(label=_('Cut first'),command=self.Content.set_cut_first)
        popup.add_command(label=_('Cut last'),command=self.Content.set_cut_last)
        popup.add_separator()
...
class CanvasContentClass:
    ...
    def plot_cut_info(self):
        ...
        for shape_nr in draw_list:
            if not(shape_nr in self.Disabled):
                ...
                start,start_ang=self.Shapes[shape_nr].get_st_en_points(0)
                self.textbox.prt(_("\nShape %d starts at %8.3f %8.3f") %(shape_nr,start.x, start.y))
    ...
    def set_cut_last(self):
        for shape_nr in self.Selected:
            if not (shape_nr in self.CutLast):
                self.CutLast.append(shape_nr)
        self.textbox.prt(_('\n\nShape: %d set to cut last')\
                             %(shape_nr))
        self.set_shapes_color(self.Selected,'cutlast')
        self.plot_cut_info() 

    def set_cut_first(self):
        for shape_nr in self.Selected:
            if not (shape_nr in self.CutFirst):
                self.CutFirst.append(shape_nr)
        self.textbox.prt(_('\n\nShape: %d set to cut first')\
                             %(shape_nr))
        self.set_shapes_color(self.Selected,'cutfirst')
        self.plot_cut_info() 
